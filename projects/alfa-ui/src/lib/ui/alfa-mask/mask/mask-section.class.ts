...
export class Action {
    public static NONE = new Action('NONE');
    public static APPLY = new Action('APPLY');
    public static SKIP = new Action('SKIP');
    public static GO_FWD = new Action('GO_FWD');
    public static GO_BACK = new Action('GO_BACK');
    public static GO_BACK_AND_DELETE = new Action('GO_BACK_AND_DELETE');
...
export class MaskSection {
    constructor(
        ...
        public sectionType: MaskSectionType | null = null,
    ) {}
    ...
    public get maxLength(): number {
        ...
        if (
            this.sectionType &&
            this.sectionType.max &&
            `${this.sectionType.max}`.length > this.section.length
        ) {
    ...
    public hasRegExp(): boolean {
        ...
        return (
            this.sectionType &&
            this.sectionType.regExp !== null &&
            this.sectionType.regExp !== undefined
        );
    }
    ...
    public checkMinMax(n: number): number | null {
        if (n === null) return null;

        if (!this.sectionType) return n;
    ...
    public removePlaceholders(txt: string): string {
    ...
    private incValue(value: string): string {
        // Next option
        if (this.hasOptions()) {
            const i = this.sectionType?.options?.indexOf(value);
            if (i === undefined || !this.sectionType?.options) throw new Error('wrong index');

            return i < this.sectionType.options.length - 1
                ? this.sectionType.options[i + 1]!
                : this.sectionType.options[0]!;
        }
        ...
        if (this.isNumeric() && this.sectionType?.max !== undefined) {
            ...
            while (res.length < this.length) res = `0${res}`;
    ...
    private decValue(value: string): string {
        ...
        if (this.hasOptions()) {
            ...
            return i > 0
                ? this.sectionType.options[i - 1]!
                : this.sectionType.options[this.sectionType.options.length - 1]!;
        }
    ...
    public autoCorrectVal(s: string): string {
        if (!this.sectionType) return s;

        if (this.hasOptions()) {
            const variant = this.sectionType.options?.find(
        ...
        if (this.isNumeric()) {
            let n: number | null = this.numericValue(res);
            ...
            if (this.settings.autoCorrect) n = this.checkMinMax(n);
    ...
    public extract(
        ...
        const section = maskValue.substring(sectionPos, i2);
        ...
        res.inSection = selStartLocal >= 0 && selStartLocal <= res.section.length;

        return res;
    }

    private skip(mv: MaskValue, selStart: number): MaskResult {
        const res: MaskResult = new MaskResult(mv.value(), Action.SKIP, mv.nextSectionPos());
        res.selStart = selStart;
        return res;
    }

    private none(mv: MaskValue): MaskResult {
        const res: MaskResult = new MaskResult(mv.value(), Action.NONE, mv.nextSectionPos());
        return res;
    }
    ...
    private apply(
        ...
        const selStartLocal = selStart - mv.sectionPos;
        ...
        if (
            ...
            selStartLocal >= newSectionValue.length - 1 &&
            direction > 0
        ) {
        ...
        res.selStart = selStart;
        res.selLength = this.settings.replaceMode && selStartLocal < this.length ? 1 : 0;
        return res;
    }
    ...
    private applyDelimiter(mv: MaskValue, selStart: number): MaskResult {
        ...
        const sv = this.autoCorrectVal(mv.section.value());

        mv.update(sv, selStart);
        mv.delimiter = this.delimiter;

        // Moving forward
        const res: MaskResult = new MaskResult(mv.value(), Action.GO_FWD, mv.nextSectionPos());
    ...
    private goBack(
        ...
        const selStartLocal = res.selStart - mv.sectionPos;

        // If we exceed minimal length...
        if (selStartLocal >= this.length && selStartLocal >= mv.section.length) res.selLength = 0;

        const newSelStartLocal = res.selStart - mv.sectionPos;
        if (newSelStartLocal < 0 && mv.before !== '') {
            res.action = byBackspace ? Action.GO_BACK_AND_DELETE : Action.GO_BACK;
        }

        if (res.selStart < 0) res.selStart = 0;

        if (res.selLength === 0 && selStart === 0) res.selLength = 0;
    ...
    private goFwd(
        ...
        selStart: number,
        selLength: number,
        appendDelimiter: boolean = false,
    ): MaskResult {
        const selStartLocal: number = selStart - mv.sectionPos;
        ...
        // We're out of bounds
        if (
            newSelStartLocal > mv.section.length ||
            (newSelStartLocal === this.maxLength && mv.after !== '')
        ) {
            // Trying to apply autoCorrection
            const v = this.autoCorrectVal(mv.section.value());

            mv.delimiter = this.delimiter;
            // After that carriage's position and next section's position could be different
            mv.update(v, newSelStartLocal);
    ...
    public applyKey(
        ...
        // Parsing the value
        const mv: MaskValue = this.extract(value, sectionPos, selStart, selLength);

        // Cursor's positioned before the section. Doing nothing
        if (selStart < sectionPos) return this.none(mv);
        ...
        if (selStartLocal === this.maxLength && this.delimiter === '' && mv.after !== '') {
            return this.skip(mv, selStart);
        }

        if (keyChar !== this.delimiter[0] || !acceptDelimiterChars) {
            if (
                this.isEmptySection() || // Empty section
        ...
        if (keyChar.length === 1) {
            ...
            if (this.hasOptions() && cValue !== mv.section.beforeChars) {
                let optionFound: string | null = null;
                this.sectionType?.options?.some((variant) => {
                    if (
                        selStartLocal < variant.length &&
                        variant.substring(0, cValue.length).toLowerCase() === cValue.toLowerCase()
                    ) {
                        optionFound = variant;
                        return true;
                    }
                ...
                if (optionFound !== null) return this.apply(mv, optionFound, selStart, 1, isLast);
            }

            // Regular expression
            if (this.hasRegExp()) {
                // New value will be:
                const nv: string = mv.section.value(keyChar);
            ...
            if (
                ...
                if (selStartLocal < this.maxLength) {
        ...
        if (keyCode === Keys.DELETE) {
            ...
            if (selStartLocal >= this.length && mv.section.afterChars === '') {
                if (mv.after === '') mv.delimiter = '';
        ...
        if (keyCode === Keys.BACKSPACE) {
            ...
            mv.section.beforeChars = mv.section.beforeChars.substring(
                0,
                mv.section.beforeChars.length - 1,
            );

            if (
                (mv.section.beforeChars.length >= this.length && mv.after === '') ||
    ...
    public setDefaultVariant(value: string, sectionPos: number): string {
        ...
        // Remove placeholders
        const s = this.removePlaceholders(mv.section.value());

        // If empty...
        if (s === '') {
            // @ts-expect-error
            const applyResult: MaskResult = this.apply(mv, this.sectionType.options[0], 0, 0);
    ...
    public selectFirst(value: string, sectionPos: number): MaskResult {
        const mv: MaskValue = this.extract(value, sectionPos, sectionPos, 0);
        const res: MaskResult = new MaskResult(mv.value(), Action.APPLY, mv.nextSectionPos());

        res.selStart = sectionPos;
        res.selLength = this.settings.replaceMode ? 1 : 0;
        if (this.isEmptySection()) res.selLength = 0;
