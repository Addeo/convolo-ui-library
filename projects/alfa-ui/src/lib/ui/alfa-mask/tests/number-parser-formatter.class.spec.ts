...
describe(`Parse integer value ${intStr}`, () => {
    const v = NumberParserFormatter.parse(intStr, intFmt, ['.', ',']);
    it("Value = 2019'", () => expect(v).toBe(2019));
});

describe(`Format positive value with separators ${intStr}`, () => {
    const v = NumberParserFormatter.format(1234, '{R4.2}', ['.', ',']);
    it("Value = '1,234.00'", () => expect(v).toBe('1,234.00'));
});

describe(`Format positive value without separators ${intStr}`, () => {
    const v = NumberParserFormatter.format(1234, '{P4.2}', ['.', ',']);
    it("Value = '1234.00'", () => expect(v).toBe('1234.00'));
});

describe(`Format integer value ${intStr}`, () => {
    const v = NumberParserFormatter.format(2019, intFmt, ['.', ',']);
    it("Value = 2019'", () => expect(v).toBe('2019'));
});

describe("Parse '1.66'", () => {
    const v = NumberParserFormatter.parse('1.66', testFmt4, ['.', ',']);
    it('Value = 1.66', () => expect(v).toBe(1.66));
});

describe(`Parse ${testStr1}`, () => {
    const v = NumberParserFormatter.parse(testStr1, testFmt1, ['.', ',']);
    it("Value = 123,456,789.01'", () => expect(v).toBe(123456789.01));
});

describe(`Parse ${testStr4}`, () => {
    const v = NumberParserFormatter.parse(testStr4, testFmt4, ['.', ',']);
    it("Value = 12345.00'", () => expect(v).toBe(12345.0));
});

describe(`Parse ${testStr2}`, () => {
    const v: number = NumberParserFormatter.parse(testStr2, '', ['.', ',']);
    it(`Value = ${testStr2}`, () => expect(v).toBe(-1.2345e6));
});

describe('Parse -$123.00', () => {
    const v: number = NumberParserFormatter.parse('-$123.00', '~${N1.2}', ['.', ',']);
    it('Value = -123', () => expect(v).toBe(-123));
});

describe(`Parse ${testStr3}`, () => {
    const v: number = NumberParserFormatter.parse(testStr3, testFmt3, [',', ' ']);
    it(`Value = ${testStr3}`, () => expect(v).toBe(123456.78));
});

describe('Parse empty string', () => {
    const v: number = NumberParserFormatter.parse('', testFmt3, [',', ' ']);
    it('Value = null', () => expect(v).toBeNull());
});

describe(`Format ${testStr1}`, () => {
    const v = 123456789.1;
    const s: string = NumberParserFormatter.format(v, '{N1.3-4}', ['.', ',']);
    it(`${v} with {1.3-4} = ${testStr3}`, () => expect(s).toBe('123,456,789.100'));
});

describe(`Format ${testStr1}`, () => {
    const v = 123456789.245;
    const s: string = NumberParserFormatter.format(v, '{+N1.2}', ['.', ',']);
...
describe('Format 0', () => {
    const v = 0;
    const s: string = NumberParserFormatter.format(v, '{1.3-4}', ['.', ',']);
    it(`${v} with {1.3-4} = 0.000`, () => expect(s).toBe('0.000'));
});

describe('Format int value', () => {
    const v = 123;
    const s: string = NumberParserFormatter.format(v, '{1.0-4}', ['.', ',']);
...
describe(`Reformat ${testStr1}`, () => {
    const s = '-123456789.25';
    const state: any = NumberParserFormatter.reformat(s, '{n1.2}', ['.', ','], 0, 0);
...
describe('Reformat -1 (convertToFormat=true)', () => {
    const s = '-1';
    const state: any = NumberParserFormatter.reformat(s, '{n1.2}', ['.', ','], 0, 0, true);
    it(`${s} with {n1.2} = '-1.00'`, () => expect(state.value).toBe('-1.00'));
});

describe('Reformat 1', () => {
    const s = '1';
    const state: any = NumberParserFormatter.reformat(s, '{n4.2}', ['.', ','], 0, 0, true);
    it(`${s} with {n4.2} = '0001.00'`, () => expect(state.value).toBe('0001.00'));
});

describe('Reformat .01', () => {
    const s = '.01';
    const state: any = NumberParserFormatter.reformat(s, '{n1.2}', ['.', ','], 0, 0, false);
    it(`${s} with {n1.2} = '0.01'`, () => expect(state.value).toBe('0.01'));
});

describe('Reformat 001.01 = 1.01', () => {
    const s = '001.01';
    const state: any = NumberParserFormatter.reformat(s, '{n1.2}', ['.', ','], 3, 6, false);
...
describe(`Reformat after backspace: ${testStr1}`, () => {
    const s = '1,312,312,311,231,23.00';
    const state: any = NumberParserFormatter.reformat(s, '{n1.2}', ['.', ','], 20, 0);
...
describe('Reformat after backspace 2: -$', () => {
    const s = '-';
    const state: any = NumberParserFormatter.reformat(s, '~${n1.2}', ['.', ','], 1, 0);
    it(s, () => expect(state.value).toBe('-'));
    it('Cursor position = ', () => expect(state.selStart).toBe(1));
});

describe('Reformat after decimal point insert ', () => {
    const s = '123,12.3,123';
    const state: any = NumberParserFormatter.reformat(s, '{n1.2-4}', ['.', ','], 0, 0);
...
describe('Can accept key', () => {
    ...
    if (NumberParserFormatter.canAcceptKey(s, null, '.', '{1-4.2}', ['.', ','], 2))
        state = NumberParserFormatter.reformat(`${s}.`, '{1-4}', ['.', ','], 0, 0);
    it(`${s} Append decimal point at the end of '123'. Result should be '123.'.`, () =>
    ...
    it(`${s} Can't append number at the end of '1230' with format {1-4.2}`, () =>
    ...
    s = '$1';
    const res11 = NumberParserFormatter.canAcceptKey(
        s,
        Keys.BACKSPACE,
        '',
        '${n1-4.2}',
    ...
    s = '-$1';
    const res11_2 = NumberParserFormatter.canAcceptKey(
        s,
        Keys.BACKSPACE,
        '',
        '~${n1-4.2}',
    ...
    s = '$1.2';
    const res12 = NumberParserFormatter.canAcceptKey(
        s,
        Keys.BACKSPACE,
        '',
        '${n1-4.2}',
    ...
    it(`${s} Can't delete decimal point if convertToFormat = true`, () =>
